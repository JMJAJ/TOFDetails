...
async def help_catto(ctx):
  ...
  embed.add_field(name="Gift",
                  ...
                  inline=False)
  embed.add_field(name="", value="Usage: !gift [character_name]", inline=False)
...
async def list_all_weapons(ctx):
  ...
  if weapon_names:
    ...
    while True:
      embed = discord.Embed(
          description=
          f"Page {current_page}/{num_pages}:\n```{chunks[current_page - 1]}```"
      )
      embed.set_footer(text="React with ◀️ or ▶️ to navigate through pages.")
...
async def format_weapon_details(ctx, weapon_data):
  ...
  if weapon_effects:
    ...
    for effect in weapon_effects:
      formatted_details += f"- {effect['title']}: {effect['description']}\n"
  ...
  if weapon_attacks:
    ...
    for attack in weapon_attacks['normals']:
      formatted_details += f"- {attack['name']}: {attack['description']}\n"
...
async def list_all_simulacrums(ctx):
  ...
  if simulacrum_data:
    ...
    while True:
      ...
      table = tabulate(current_simulacrums, headers=headers, tablefmt='plain')
      embed = discord.Embed(
...
def format_simulacrum_details(simulacrum_data):
  ...
  formatted_details = f"Simulacrum Name: {simulacrum_data['simulacrumName']}\n"
...
async def list_all_servants(ctx):
  ...
  if servant_data:
    ...
    table = tabulate(table_data, headers=['Servant Name'], tablefmt='plain')
...
def format_servant_details(servant_data):
  ...
  for item in upgrade_items:
    ...
    exp = item['exp']
    formatted_details += f"- {material_name} (EXP: {exp}): {material_description}\n"
...
async def fetch_character_details(ctx, character_name):
  ...
  character_data = fetch_character_data(api_url)
  if character_data:
    selected_character = find_character_by_name(character_data, character_name)
...
async def format_character_details(character_data):
  ...
  formatted_details += f"Released: {'Yes' if character_data.get('isReleased', False) else 'No'}\n"
  ...
  formatted_details += f"Affiliation: {character_data.get('affiliation', 'N/A')}\n"

  formatted_details += f"Avatar: [Click Here]({character_data.get('assetsA0', {}).get('avatar', 'N/A')})\n"
  formatted_details += f"Character Artwork: [Click Here]({character_data.get('assetsA0', {}).get('characterArtwork', 'N/A')})\n"
  ...
  for banner in character_data.get('banners', []):
    formatted_details += f"- Simulacrum ID: {banner.get('simulacrumId', 'N/A')}\n"
  ...
  for awakening in character_data.get('awakening', []):
    ...
    formatted_details += f"- Description: {awakening.get('description', 'N/A')}\n"
  ...
  for guidebook in character_data.get('guidebook', []):
    ...
    formatted_details += f"- Description: {guidebook.get('description', 'N/A')}\n"
...
async def list_all_achievements(ctx):
  ...
  if achievement_data:
    table_data = [(achievement['name'], ) for achievement in achievement_data]
    table = tabulate(table_data, tablefmt='plain')

    MAX_MESSAGE_LENGTH = 2000 - len("List of available achievements:\n``````")
    chunks = [
        table[i:i + MAX_MESSAGE_LENGTH]
        for i in range(0, len(table), MAX_MESSAGE_LENGTH)
    ]
...
async def fetch_achievement_details(ctx, achievement_name):
  ...
  if achievement_data:
    ...
    if selected_achievement:
      formatted_details = format_achievement_details(selected_achievement)
      await ctx.send(formatted_details)
    else:
